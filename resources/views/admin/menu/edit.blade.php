...
@section('page_title', 'Редактировать пункт меню')

@section('content')
<form action="{{ route('admin.menu.update', $menu) }}" method="POST" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    <div>
    ...
    <div>
        ...
        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $menu->sort_order) }}" min="1"
               class="w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
    ...
    <label class="flex items-center gap-2">
        ...
        <input type="checkbox" name="open_new_tab" value="1" {{ old('open_new_tab', $menu->open_new_tab) ? 'checked' : '' }}
               class="rounded border-stone-300 text-amber-500 focus:ring-amber-500">
