...
@section('title', 'Редактировать стоимость')
@section('page_title', 'Редактировать стоимость')

@section('content')
<form action="{{ route('admin.prices.update', $price) }}" method="POST" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="3"
                  class="w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">{{ old('description', $price->description) }}</textarea>
        @error('description')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="price_text" class="block text-sm font-medium text-stone-700 mb-1">Цена (текстом)</label>
        <input type="text" name="price_text" id="price_text" value="{{ old('price_text', $price->price_text) }}"
               class="w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
    ...
    <div class="flex items-center gap-2">
        <input type="checkbox" id="is_featured" name="is_featured" value="1"
               ...
               {{ old('is_featured', $price->is_featured) ? 'checked' : '' }}>
    ...
    <div>
        ...
        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $price->sort_order) }}" min="1"
               class="w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
