...
<div class="mb-4 flex justify-end">
    <a href="{{ route('admin.articles.create') }}" class="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-stone-900 font-medium rounded-lg transition">Добавить статью</a>
</div>
        ...
        <tbody>
            @forelse($articles as $item)
                <tr class="border-b border-stone-100 hover:bg-stone-50/50">
                    <td class="px-4 py-3">
                        <a href="{{ route('admin.articles.edit', $item) }}" class="text-stone-800 hover:text-amber-600 font-medium">{{ Str::limit($item->title, 50) }}</a>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right">
                        <a href="{{ route('admin.articles.edit', $item) }}" class="text-amber-600 hover:text-amber-700 text-sm mr-3">Изменить</a>
                        <form action="{{ route('admin.articles.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Удалить статью?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="4" class="px-4 py-8 text-center text-stone-500">Статей пока нет</td>
                </tr>
            @endforelse
        </tbody>
    </table>
