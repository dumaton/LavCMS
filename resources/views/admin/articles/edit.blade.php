...
@section('content')
<form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="max-w-2xl space-y-4">
    @csrf
    @method('PUT')
    <div>
    ...
    <div>
        ...
        <input type="text" name="excerpt" id="excerpt" value="{{ old('excerpt', $article->excerpt) }}"
               class="w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
    ...
    <div>
        ...
        @if($article->image)
            <div class="mb-2">
                <img src="{{ asset('storage/' . $article->image) }}" alt="{{ $article->title }}" class="h-24 rounded border border-stone-200 object-cover">
            </div>
        @endif
    ...
    <div>
        ...
        <textarea name="body" id="body" rows="10" required
                  class="js-richtext w-full px-3 py-2 border border-stone-300 rounded-lg focus:border-amber-500 focus:ring-1 focus:ring-amber-500">{{ old('body', $article->body) }}</textarea>
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('admin.articles.index') }}" class="px-4 py-2 border border-stone-300 rounded-lg hover:bg-stone-50 transition">Отмена</a>
    </div>
</form>
@endsection
