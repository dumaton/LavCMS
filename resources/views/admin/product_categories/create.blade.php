...
<form action="{{ route('admin.product-categories.store') }}" method="POST" enctype="multipart/form-data" class="max-w-xl space-y-6">
    ...
    <div>
        ...
        <p class="text-xs text-stone-500 mt-1">Оставьте пустым — сгенерируется из названия. Только латиница, цифры и дефис.</p>
        @error('slug')
            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        <textarea name="svg_icon" rows="4"
                  ...
                  placeholder="<svg>...">{{ old('svg_icon') }}</textarea>
    ...
    <div>
        ...
        <input type="number" name="sort_order" value="{{ old('sort_order', $nextSortOrder ?? 1) }}" min="1"
