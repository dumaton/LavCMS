...
@section('content')
<h1 class="text-3xl font-bold text-stone-900 mb-6">Статьи</h1>

<div class="space-y-6">
    @forelse($articles as $item)
        <article class="bg-white rounded-xl border border-stone-200 p-6 shadow-sm hover:border-amber-200 transition">
            <a href="{{ route('articles.show', $item) }}" class="text-xl font-semibold text-stone-800 hover:text-amber-600">{{ $item->title }}</a>
            @if($item->excerpt)
